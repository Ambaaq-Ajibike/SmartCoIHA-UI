  ...
  return (
    <aside className="flex h-screen w-64 flex-col border-r border-emerald-100 bg-white">
      <div className="flex h-16 items-center gap-2 border-b border-emerald-100 px-6">
        ...
        <span className="font-display text-lg font-semibold text-primary">Institution Panel</span>
      </div>

      <nav className="flex-1 overflow-y-auto px-3 py-4">
        <ul className="space-y-1">
          {navItems.map(({ href, label, icon: Icon }) => {
            const isActive = href === "/dashboard/manager" ? pathname === href : pathname.startsWith(href);
              ...
              <li key={href}>
                <Link
                  ...
                  className={`flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-medium transition-colors ${
                    isActive ? "bg-primary text-white" : "text-ink hover:bg-emerald-50 hover:text-primary"
                  }`}
                >
      ...
      <div className="relative border-t border-emerald-100 p-3">
        ...
        >
          ...
          <div className="min-w-0 flex-1">
            ...
            <p className="truncate text-xs text-muted">{user?.institutionName || "Institution"}</p>
          </div>
          <ChevronUp
